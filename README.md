...
2018년 6월부터 9월까지 약 3개월간 `암호분석경진대회`, `국가암호공모전`에 참가했습니다. `암호분석경진대회`와 `국가암호공모전`은 각 3명의 팀으로 구성되어있습니다. 저는 두 공모전 모두 `팀장`으로 참여했으며, 매주 `진행상황 보고`와 `PT발표`를 통해 성공적인 프로젝트로 이끌었습니다. 두 공모전에서 `블록암호`, `해시함수`, `대칭키암호`와 관련된 문제를 맡았습니다.
